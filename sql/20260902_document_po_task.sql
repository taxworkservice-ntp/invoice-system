-- Documents: customer PO reference + task name (self-contained).
-- documents.customer_po_number / documents.task_name — optional free text,
-- printed on classic V2 DN / tax invoices (เลขที่ใบสั่งซื้อ / PO NO. and
-- ชื่องาน / JOB NAME rows in the document meta block).
-- Also recreates create_invoice_from_sources and create_deal_document so
-- inserts persist the new columns (both RPCs have explicit column whitelists).
-- Run this whole file in the Supabase SQL editor.

alter table public.documents add column if not exists customer_po_number text;
alter table public.documents add column if not exists task_name text;
