-- Add is_blank_form flag for delivery notes issued as "blank forms" (handwritten at delivery, filled in later).
-- The flag marks a DN draft whose printed line-item qty/price cells are left empty for manual entry.

alter table public.documents add column if not exists is_blank_form boolean not null default false;
