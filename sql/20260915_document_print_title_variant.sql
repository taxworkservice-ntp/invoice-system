-- documents.print_title_variant — printed-header title preset for a Thai tax
-- invoice (ใบกำกับภาษี) that also serves as another document, e.g.
-- "ใบกำกับภาษี/ใบส่งของ/ใบแจ้งหนี้". PRINT-ONLY: doc_type stays "invoice"
-- everywhere (lists, reports, status) — only the rendered header changes.
-- See src/lib/docLabels.ts (PRINT_TITLE_PRESETS + printTitle).
--
-- Both insert RPCs use explicit column whitelists, so they are recreated here
-- to persist the new column (same pattern as sql/20260914_dn_so_header.sql).
-- Run this whole file in the Supabase SQL editor before deploying the client.

alter table public.documents add column if not exists print_title_variant text;
...
begin
  ...
  insert into public.documents (
    ...
    nullif(p_document->>'print_title_variant', '')
...
begin
  ...
  insert into public.documents (
    ...
    customer_po_number, task_name, dn_so_header, print_title_variant
  ) values (
