-- Per-document font-size override for the Classic V2 template (self-contained).
-- documents.print_font_scale: 'small'|'normal'|'large'|'xlarge'|'xxlarge'|'xxxlarge'
-- NULL = follow the workspace default (client_profiles.classic_v2_font_scale).
-- Also recreates create_deal_document so inserts persist the new column
-- (the RPC has an explicit column whitelist).
-- Run this whole file in the Supabase SQL editor.

alter table public.documents add column if not exists print_font_scale text;
