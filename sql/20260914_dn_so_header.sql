-- DN SO group header (self-contained, opt-in for ~5% of customers).
-- documents.dn_so_header — free-text SO / Part-no line typed on the delivery
-- note form (e.g. "SO7944758301/Z033248905 Part no.25120021 (เห็ด)").
-- Printed on Classic V2 only: as group "1." above the DN lines (children
-- 1.1, 1.2…), and frozen onto tax invoices billed from the DN as a second
-- line under the "อ้างอิงใบส่งของ DN-…" group header.
-- invoice_delivery_notes.so_header — snapshot copied at billing time, so
-- editing the DN later never changes an already-issued invoice reprint.
-- Also recreates create_deal_document and create_invoice_from_sources so
-- inserts persist the new columns (both RPCs have explicit column whitelists).
-- Run this whole file in the Supabase SQL editor.

alter table public.documents add column if not exists dn_so_header text;
alter table public.invoice_delivery_notes add column if not exists so_header text;

-- Backfill links from their DN (guarded: on a fresh database every
-- dn_so_header is NULL, so zero rows update and the invoice touch-trigger
-- never fires — no PDF cache invalidation for untouched invoices).
update public.invoice_delivery_notes j
set so_header = d.dn_so_header
from public.documents d
where d.id = j.delivery_note_id
  and d.dn_so_header is not null
  and btrim(d.dn_so_header) <> ''
  and (j.so_header is null or btrim(j.so_header) = '');
...
begin
  ...
  insert into public.documents (
    ...
    show_dn_variance, dn_appendix, customer_po_number, task_name, converted_from_id
  ) values (
  ...
  -- Link delivery-note sources. so_header is a frozen snapshot of the DN's
  -- dn_so_header at billing time — later DN edits never change this invoice.
  insert into public.invoice_delivery_notes (
    invoice_id, delivery_note_id, user_id, delivery_note_number, issue_date,
    subtotal, vat_amount, total_amount, so_header
  )
  select
    ...
    nullif(btrim(coalesce(d.dn_so_header, '')), '')
  from public.documents d
...
begin
  ...
  insert into public.documents (
    ...
    customer_po_number, task_name, dn_so_header
  ) values (
    ...
    nullif(p_document->>'customer_po_number', ''),
    nullif(p_document->>'task_name', ''),
    nullif(btrim(coalesce(p_document->>'dn_so_header', '')), '')
  ) returning id into v_document_id;
