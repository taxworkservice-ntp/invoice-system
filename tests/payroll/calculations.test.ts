...
describe("payroll calculations", () => {
  ...
  it("exempts age-60 hires from SSO but keeps progressive withholding", () => {
    const exempt = calculateBreakdown(input({ base_salary: 30000, sso_exempt: true }), settings, 1);
    expect(exempt.sso_employee).toBe(0);
    expect(exempt.sso_employer).toBe(0);
    // Progressive ภ.ง.ด.1 on 30,000 with zero SSO deduction — NOT the 3% flat rate (900).
    expect(exempt.withholding_tax).toBeCloseTo(calculateMonthlyWithholdingTax(30000, 0), 2);
    expect(exempt.withholding_tax).not.toBe(30000 * 0.03);

    const contract = calculateBreakdown(
      input({ base_salary: 30000, sso_registered: false }),
      settings,
      1,
    );
    expect(contract.withholding_tax).toBe(30000 * 0.03);
  });
        ...
        ot_entries: [
          {
            ...
            type: "normal",
            multiplier: "1.5" as unknown as number,
          },
        ],
...
describe("calculation customization", () => {
  ...
  it("pays daily-staff OT on daily wage ÷ hours-per-day (statutory basis, no ÷30 step)", () => {
    ...
    const result = calculateBreakdown(
      input({
        ...
        base_salary: 300,
        days_worked: 7,
        ot_entries: [{ hours: 15, type: "normal", multiplier: 1.5 }],
      }),
      settings,
      8,
    );
  ...
  it("suggests leave pro-ration via absent days (leaver mid-month)", () => {
    ...
    expect(
      suggestLeaveProrate("2024-02-20", { ...settings, prorate_mode: "actual_days" }, 2024),
  ...
  it("applies the suggested absence and reaches half-month pay", () => {
    ...
    const result = calculateBreakdown(
      input({ base_salary: 30000, absent_days: suggestion.absent_days }),
      settings,
      8,
    );
...
describe("worker type: not SSO-registered (ค่าจ้างทำของ / PND3)", () => {
  it("withholds flat 3% of gross and skips SSO entirely", () => {
    const result = calculateBreakdown(
      input({ base_salary: 20000, sso_registered: false }),
      settings,
      8,
    );
    expect(result.gross_pay).toBe(20000);
  ...
  it("keeps SSO employees on the existing PIT + SSO path", () => {
    ...
    const ssoExplicit = calculateBreakdown(
      input({ base_salary: 20000, sso_registered: true }),
      settings,
      8,
    );
...
describe("withholding tax (annualized PND1 with proper deductions)", () => {
  ...
  it("applies the 5% and 10% brackets on a mid income (50k/mo)", () => {
    ...
    expect(calculateMonthlyWithholdingTax(50000, calculateSSO(50000).employee)).toBeCloseTo(
      1704.17,
      2,
    );
  });
...
describe("manual per-day absence rate override", () => {
  it("uses the override rate instead of the derived rate", () => {
    ...
    const result = calculateBreakdown(
      input({ absent_days: 2, absence_daily_rate: 1200 }),
      settings,
      8,
    );
  ...
  it("ignores the override when absent_days is zero", () => {
    const result = calculateBreakdown(
      input({ absent_days: 0, absence_daily_rate: 1200 }),
      settings,
      8,
    );
    expect(result.absence_deduction).toBe(0);
  ...
  it("respects the absence_deduction=false kill switch over any override", () => {
    ...
    expect(
      calculateBreakdown(input({ absent_days: 2, absence_daily_rate: 1200 }), off, 8)
        .absence_deduction,
    ).toBe(0);
  });
