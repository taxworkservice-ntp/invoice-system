-- Reference-mode billing marks sources converted (status-driven, like
-- QuickBooks/Odoo unbilled queues).
--
-- Gap found on live data (บริษัท เทสท์ โค จำกัด): the DN→INV combine flow can
-- bill in "reference mode" — one printed invoice line per delivery note,
-- carrying its total, linked via source_document_id with source_line_item_id
-- NULL. Because the coverage check only matches line-to-line billing, such
-- DNs stayed 'sent': they kept appearing in "DN รอออกบิล" and in the combine
-- picker, so the same delivery note could be billed twice.
--
-- Fix (create_invoice_from_sources):
--   * A source counts as fully billed when an ACTIVE invoice line references
--     it in ref-mode (source_document_id = source, source_line_item_id NULL).
--     Ref-mode always bills the whole document — there is no partial variant.
--   * Line-to-line coverage math unchanged for partial billing.
--
-- Data repair (idempotent, same rule as the function): delivery notes and
-- quotations still 'sent' that are referenced by an active invoice's ref-mode
-- line flip to 'converted'; quotations whose delivery notes are all converted
-- follow (same cascade rule the function applies).
...
begin
  ...
  foreach v_source_id in array p_source_ids loop
    ...
    if v_source.customer_id is not null and v_source.customer_id <> v_customer_id then
      raise exception 'Source % belongs to a different customer — one invoice per customer',
        coalesce(v_source.doc_number, v_source_id::text);
    end if;
  ...
  -- Resolve the deal: explicit -> the sources' single shared deal -> create
  -- one. Multiple source deals are allowed (periodic billing); the invoice
  -- then lives in a new deal while each source stays in its own.
  ...
  -- Flip each source: converted when fully billed. Ref-mode billing (an
  -- active invoice line referencing the whole source with source_line_item_id
  -- NULL) always bills the entire document — treat it as full coverage so the
  -- source drops out of every unbilled queue. Otherwise fall back to
  -- line-to-line coverage, which supports partial billing.
  foreach v_source_id in array p_source_ids loop
    if exists (
      select 1
      from public.document_line_items li
      join public.documents d on d.id = li.document_id
      where li.source_document_id = v_source_id
        and li.source_line_item_id is null
        and d.doc_type = 'invoice'
        and d.status not in ('voided', 'draft')
    ) then
      v_all_covered := true;
...
-- ---------------------------------------------------------------------------
-- Data repair: sources left in 'sent' by ref-mode invoices created before
-- this fix. Idempotent — the WHERE clauses make re-runs no-ops.
-- ---------------------------------------------------------------------------

-- 1. Delivery notes / quotations referenced by an active invoice's ref-mode
--    line are fully billed -> converted.
with ref_billed as (
  select distinct li.source_document_id as doc_id
  from public.document_line_items li
  join public.documents inv on inv.id = li.document_id
  where li.source_document_id is not null
    and li.source_line_item_id is null
...
set status = 'converted'
where d.status = 'sent'
  and d.doc_type in ('delivery_note', 'quotation')
  and d.id in (select doc_id from ref_billed);

-- 2. Quotation cascade: all delivery notes converted -> quotation done.
...
where q.doc_type = 'quotation'
  ...
  and exists (
    select 1 from public.documents dn
    where dn.user_id = q.user_id
      and dn.doc_type = 'delivery_note'
      and dn.converted_from_id = q.id
  )
  and not exists (
    select 1 from public.documents dn
    where dn.user_id = q.user_id
