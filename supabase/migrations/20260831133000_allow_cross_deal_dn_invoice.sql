-- Allow "รวมใบส่งของ → ออกบิล" (DN→INV) to combine delivery notes from
-- multiple deals of the SAME customer — the periodic end-of-month billing
-- flow this form advertises (ใช้สำหรับออกบิลรายรอบ เช่น สิ้นเดือน).
--
-- The old guard rejected any mix of deals. It existed because the flip step
-- re-parented every source onto the invoice's deal (deal_id = v_deal_id),
-- which would silently pull DNs out of their original deals, breaking
-- delivery progress and per-deal histories. Combining itself is safe:
-- invoice ↔ DN traceability lives in invoice_delivery_notes, and remaining-
-- to-bill math sums across all invoices referencing each source line.
--
-- New behavior:
--   * Sources must belong to the invoice's customer (hardened — the form
--     already filters by customer, now the server enforces it too).
--   * Deal resolution: explicit deal_id → the sources' single shared deal →
--     a NEW deal for the billing run (title from p_document, the form sends
--     the customer name).
--   * Sources are never re-parented: deal_id = coalesce(d.deal_id, v_deal_id)
--     adopts only deal-less sources; DNs stay in their original deals with
--     status converted as before.
--   * Quotation cascade (QT whose DNs are all converted) unchanged.
...
begin
  ...
  -- then lives in a new deal while each DN stays in its own.
  ...
  -- Never re-parent a source out of its own deal — adopt deal-less sources
  -- into the invoice's deal only.
  foreach v_source_id in array p_source_ids loop
    select not exists (
      select 1
