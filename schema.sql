...
create table client_members (
  ...
  permissions       jsonb,
  custom_role_id    uuid references client_roles(id) on delete set null,
  created_at        timestamptz not null default now(),
  updated_at        timestamptz not null default now(),
...
create table client_permission_audit (
  id                uuid primary key default uuid_generate_v4(),
...
create index idx_client_members_custom_role
...
create index idx_client_permission_audit_workspace
...
create table client_profiles (
  ...
  logo_size             text default 'square', -- small|square|medium|rectangle|large (see LOGO_SIZE_OPTIONS)
...
-- pdf_template union: 'classic' (v1) retired — only modern + classic_v2 remain
-- (see sql/20260923_remove_pdf_template_classic.sql)
...
-- Add payroll to feature_key check
alter table client_features drop constraint if exists client_features_feature_key_check;
alter table client_features add constraint client_features_feature_key_check
...
-- Payroll history and employee snapshots
