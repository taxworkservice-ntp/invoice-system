...
export function TopBar({ title, showBack, onBack, action, width = "data" }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-card-border bg-white/90 backdrop-blur-sm">
      <div className={`mx-auto flex h-14 w-full items-center justify-between gap-2 px-4 sm:px-5 lg:px-8 ${contentClass[width]}`}>
        <div className="flex min-w-0 flex-1 items-center gap-2">
          {showBack && (
            <button
              onClick={() => (onBack ? onBack() : navigate(-1))}
              aria-label="ย้อนกลับ"
              className="-m-1.5 text-ink-500 hover:text-ink-700 p-2 rounded-control hover:bg-ink-50 transition-colors"
            >
              <ArrowLeft className="w-5 h-5" />
            </button>
          )}
          <h1 className="truncate text-body font-semibold text-ink-800 tracking-tight">{title}</h1>
        </div>
