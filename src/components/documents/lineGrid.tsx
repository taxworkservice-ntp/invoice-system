...
export function numericInputClass(highlight: boolean) {
  return `w-full rounded-control border bg-white px-2 py-1.5 text-right text-body tabular-nums text-ink-900 outline-none transition-colors ${
    highlight
...
export function LineGridHeaderRow() {
  return (
    <div
      className={`grid ${LINE_GRID_COLS} items-end gap-x-2 border-b border-card-border px-3 pb-1 pt-2 text-label font-medium text-ink-400`}
    >
      <span>รายการ</span>
