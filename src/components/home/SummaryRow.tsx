    ...
    <div className="grid max-w-row grid-cols-2 sm:grid-cols-4 gap-2">
      {items.map((item) => {
        ...
        return (
          <Card
            key={item.label}
            className={`min-h-[78px] border-[0.5px] p-3 cursor-pointer ${active ? "border-primary bg-blue-50/40" : ""}`}
            onClick={() => onCardTap(item.preset)}
          >
            <div className="flex items-start justify-between gap-2">
              <div
                className={`text-subtitle font-semibold tabular-nums leading-none ${item.alert ? "text-danger" : "text-ink-900"}`}
              >
                {item.primary === "count" ? item.count : `฿ ${formatCurrency(item.value)}`}
              </div>
              {item.primary !== "count" && item.count != null && (
                <div
                  className={`text-label tabular-nums ${item.alert ? "text-danger" : "text-ink-500"}`}
                >
                  {item.count} รายการ
                </div>
              )}
            ...
            {item.hint && (
              <div className="mt-0.5 text-label leading-4 text-ink-400">{item.hint}</div>
            )}
          </Card>
        );
      })}
