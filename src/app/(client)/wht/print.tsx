...
function buildImages(profile: ClientProfile, hideSignature = false, hideStamp = false): ImageOverlay[] {
  ...
  if (stampSrc && !hideStamp) {
    images.push({ name: "stamp", src: getProxiedImageUrl(stampSrc), top: 1870, left: 1265, width: 139, height: 139 });
  }
  return images;
}
...
function PndPage({
  ...
  const images = buildImages(profile, hideSignature, hideStamp);
    ...
    >
      ...
      <div style={{ marginTop: "8mm", display: "flex", justifyContent: "flex-end", alignItems: "flex-end" }}>
        <div style={{ textAlign: "center" }}>
          {profile.signature_url && !hideSignature && (
...
export default function WhtPrintPage() {
  ...
  const hideSignature = searchParams.get("hideSignature") === "1";
